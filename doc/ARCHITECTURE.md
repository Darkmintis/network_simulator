...
`network_simulator` is a Flutter **plugin** that simulates real-world network conditions using a **local Android VPN**. It does **not** fake HTTP delays in Dart.
...
3. TUN with `addAllowedApplication(hostPackage)` (app-scoped; abort if this fails)
4. Parse IPv4 → TCP/UDP session managers → protected sockets
5. Shape upload/download independently on worker threads (TUN reader stays non-blocking)
...
- IPv4 TCP/UDP only; ICMP and IPv6 are not shaped in the MVP stack
